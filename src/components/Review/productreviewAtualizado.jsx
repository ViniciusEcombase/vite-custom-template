...
// Mock useFetch hook to simulate the original API integration
const useFetch = ({ baseURL, timeout, retries, cache, defaultHeaders }) => {
  const get = async (endpoint) => {
    // Mock API response that matches your expected data structure
    return {
      data: [
        {
          average_rating: 4.3,
          ...
          ai_summary: {
            ...
            negative_themes: ['Size Issues', 'Color Variation'],
            summary_text:
              'Customers generally love this product for its quality and value. The main concerns are around sizing accuracy and slight color variations from photos.',
            last_analyzed_at: '2024-08-10T10:30:00Z',
          },
          ratings_count: {
            5: 65,
            4: 32,
          ...
          reviews: [
            ...
            },
            {
              ...
              content:
                'Overall satisfied with the purchase. Minor issues with sizing but customer service was helpful.',
            ...
            },
            {
              ...
              content:
                'The product is okay but the color was not exactly as shown in the pictures. Still usable though.',
  ...
  const post = async (endpoint, data) => {
    // Mock API response for creating reviews
  ...
  storage: {
    from: (bucket) => ({
      upload: async (filePath, file, options) => {
        // Mock file upload
        console.log('Uploading file:', filePath, file.name, options);
        return {
          data: { path: filePath },
          error: null,
        };
...
const ProductReviews = () => {
  ...
  const uploadFile = async (bucketName, filePath, file) => {
    ...
    try {
      // Mock the upload for demo purposes
      console.log(`Mock upload: ${file.name} to ${bucketName}/${filePath}`);
  ...
  const createReviewMedia = async (reviewId, mediaUrl, type) => {
    ...
    const response = await api.post('/review_media', mediaData);
    return response;
  };
  ...
  const handleCreateReview = async () => {
    ...
    try {
      ...
      const reviewData = {
        variant_id: 1, // You may want to pass this as a prop
        customer_id: 1, // You may want to get this from user context
